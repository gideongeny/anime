...
(function ($) {
    ...
    function showToast(msg) {
        if ($('#toast-container').length === 0) $('body').append('<div id="toast-container"></div>');
        const toast = $(`<div class="anime-toast"><i class="fa fa-info-circle"></i> ${msg}</div>`);
    ...
    function renderSlider(container, data) { if (!data || data.length === 0) { container.html('<div class="text-white-50 p-5">No data available.</div>'); return; } container.empty(); data.forEach(anime => { const img = window.TmdbAPI.getImageUrl(anime.poster_path, 'w500'); container.append(`<div class="item"><div class="product__item"><a href="anime-details.html?id=${anime.id}&tmdb=1"><div class="product__item__pic set-bg" style="background-image: url('${img}')"><div class="ep">${(anime.vote_average || 0).toFixed(1)}</div><div class="view"><i class="fa fa-play"></i></div></div></a><div class="product__item__text mt-3"><h5 class="text-truncate"><a href="anime-details.html?id=${anime.id}&tmdb=1">${anime.name || anime.title}</a></h5></div></div></div>`); }); container.owlCarousel({ loop: true, margin: 30, dots: false, nav: true, navText: ["<span class='arrow_carrot-left'></span>", "<span class='arrow_carrot-right'></span>"], responsive: { 0: { items: 2 }, 576: { items: 3 }, 992: { items: 4 }, 1200: { items: 5 }, 1600: { items: 6 } } }); }
    async function loadTrending() { const p1 = await window.TmdbAPI.getTrendingAnime(1); const p2 = await window.TmdbAPI.getTrendingAnime(2); renderSlider($('#trending-list'), [...p1, ...p2]); }
    async function loadPopular() { const p1 = await window.TmdbAPI.getPopularAnime(1); const p2 = await window.TmdbAPI.getPopularAnime(2); renderSlider($('#popular-list'), [...p1, ...p2]); }
    async function loadGenreSections() { const container = $('#genre-sections-container'); const genres = [ { id: 10765, name: 'Sci-Fi & Fantasy' }, { id: 35, name: 'Comedy' }, { id: 28, name: 'Action' }, { id: 16, name: 'Animation' }, { id: 18, name: 'Drama' }, { id: 9648, name: 'Mystery' }, { id: 10749, name: 'Romance' } ]; for (const genre of genres) { const sectionId = `genre-list-${genre.id}`; container.append(`<div class="trending__product mt-5"><div class="section-title-wrap"><div class="section-title"><h4>${genre.name} Shows</h4></div><a href="categories.html?genre=${genre.id}" class="primary-btn">View All</a></div><div class="row"><div class="col-12"><div class="owl-carousel" id="${sectionId}"></div></div></div></div>`); const p1 = await window.TmdbAPI.getAnimeByGenre(genre.id, 1); const p2 = await window.TmdbAPI.getAnimeByGenre(genre.id, 2); renderSlider($(`#${sectionId}`), [...p1, ...p2]); } }
    async function loadCategories(search, page, genre) { const container = $('#categories-container').html('<div class="col-12 text-center p-5"><div class="spinner-border text-danger"></div></div>'); try { let data = []; if (search) data = await window.TmdbAPI.discoverAnime(search, page); else if (genre) data = await window.TmdbAPI.getAnimeByGenre(genre, page); else data = await window.TmdbAPI.getPopularAnime(page); container.empty(); if (!data || data.length === 0) { container.html('<div class="col-12 text-center text-white-50 p-5">No titles found.</div>'); return; } data.forEach(anime => { container.append(`<div class="col-lg-3 col-md-4 col-6 mb-4"><div class="product__item"><a href="anime-details.html?id=${anime.id}&tmdb=1"><div class="product__item__pic set-bg" style="background-image: url('${window.TmdbAPI.getImageUrl(anime.poster_path, 'w500')}')"></div></a><div class="product__item__text mt-3"><h5 class="text-truncate"><a href="anime-details.html?id=${anime.id}&tmdb=1">${anime.name || anime.title}</a></h5></div></div></div>`); }); } catch(e) { container.html('<div class="col-12 text-center text-white-50 p-5">Failed to load categories.</div>'); showToast('Error loading categories'); } }
